...
| A4 | 探针内调用目标进程函数 | tracer staged 写入远程 `call_actions` 表；payload entry handler 在目标进程内调用无参 callee 并写 `ZT_TRACE_EVENT_CALL`；action slot 使用独立分配并处理 probe id 取模碰撞；call event 携带真实 `callee_addr` | `test_probe_lifecycle` 中 call action 子测试和 `id=1/id=33` 槽位碰撞回归；`test_probe_hot_update` 验证 A/B call action 热更新、callee symbol cache 和返回值命名空间 | 已覆盖 |
| A5 | 探针热更新 | filter、call action、enable/disable 状态均可在目标继续运行时更新；call action 用 `enabled` commit bit 避免半写入状态；状态切换保留 probe 元数据和 trampoline slot | `test_probe_hot_update` 五阶段验证 95 对 entry/return、70 条 staged call event、false/disabled 阶段零泄漏、slot 不变；live 子测试验证运行中 A/B/clear/A 切换；100 轮压力复测通过 | 已覆盖当前可变行为参数 |
...
| 目标退出窗口 | `zt_injector_poll_events()` 记录 `target_exited`；`zt_trace_poll()` 读 buffer 失败后检查进程退出状态和 trace buffer 映射是否消失；退出后重复 poll 返回完成态；`test_poll_exit_race` 默认 50 轮，压力复测 1000 轮通过 | leader exit、线程表清空、短退出 race、远程 trace buffer 映射消失和重复 poll 都会归类为正常退出 |
...
- 单次用户态 probe 额外开销 `169.51 ns < 1000 ns`，同时低于当前项目优化目标 `200 ns`
