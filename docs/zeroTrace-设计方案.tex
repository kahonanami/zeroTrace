...
\hypersetup{
  hidelinks,
  pdftitle={zeroTrace 轻量级用户态动态探针设计与实现},
  pdfauthor={动感光波biubiubiu}
}

\pagestyle{fancy}
\fancyhf{}
\lhead{zeroTrace 技术报告}
...
\newcommand{\todoexp}{\textcolor{ztorange}{待复跑}}
\newcommand{\doneexp}{\textcolor{ztgreen}{已通过}}
\newcommand{\partialexp}{\textcolor{ztblue}{待补充}}
...
\begin{titlepage}
  \centering
  \vspace*{2.0cm}
  {\zihao{2}\bfseries zeroTrace：轻量级用户态动态探针设计与实现\par}
  \vspace{0.8cm}
...
本文档按技术报告的结构整理 zeroTrace 的设计、实现和实验。后文会对照赛题要求说明支持情况，并依次介绍总体架构、关键实现、正确性测试和性能评测。尚未补齐的平台运行记录单独列出，和已有结果分开说明。
...
\midrule
\endhead
kernel uprobe / eBPF & 用户态函数命中后进入内核，事件由内核侧处理 & 内核生态成熟，能与 perf、ftrace、BPF map 等工具组合 & 命中路径会发生用户态到内核态切换；zeroTrace 把 entry/return 处理放在目标进程内，事件写入共享 trace buffer，本轮 x86\_64 benchmark 中额外开销均值为 174.45 ns/call。\\
...
浮点上下文采用紧凑快照。x86\_64 stub 手工保存 \code{xmm0..xmm7}，aarch64 stub 保存用于参数和返回值展示的 FP/SIMD 槽位，同时保留 \code{fpcr/fpsr/nzcv}。payload 读取统一的 \code{fp\_args[]} 槽位，上层格式化逻辑只处理统一后的参数槽。相比完整 512 字节 FPU 状态，紧凑快照只记录日志展示需要的寄存器，降低热路径写入量。
...
本节数据来自 x86\_64 开发机上的 benchmark 报告。
本轮通过 Makefile 的 benchmark 目标运行，脚本检测到 tracingfs 中的 uprobe 配置入口，因此包含 kernel uprobe 对照数据。

\begin{table}[H]
\centering
\caption{x86\_64 实验环境}
\label{tab:env}
...
架构 & x86\_64\\
Kernel & 5.10.240-zeroVisor\\
编译入口 & \code{make}\\
测试入口 & \code{make test}\\
性能入口 & \code{make benchmark}\\
LaTeX 编译入口 & \code{make paper}\\
\bottomrule
\end{tabularx}
\end{table}
...
测试记录覆盖了 F1--F7；A1 中的 x86\_64 后端已经跑完整套测试，aarch64 后端已有构建选择检查和后端代码。正式提交前，还需要在 ARM 机器上用 \code{ARCH=aarch64} 复跑 \code{make test} 与 \code{make benchmark}。
...
\begin{table}[H]
\centering
\caption{最新 x86\_64 benchmark 结果}
\label{tab:benchmark}
...
repeats & 5\\
baseline total ns mean & 61,239,703 ns\\
baseline per call mean & 61.24 ns\\
uprobe total ns mean & 1,994,438,922 ns\\
uprobe per call mean & 1,994.44 ns\\
uprobe overhead/call mean & 1,933.20 ns\\
uprobe overhead/call min/max & 1,844.99 ns / 1,990.67 ns\\
uprobe overhead/call stdev & 58.33 ns\\
ztrace total ns mean & 235,684,856 ns\\
ztrace per call mean & 235.68 ns\\
ztrace overhead/call mean & 174.45 ns\\
ztrace overhead/call min/max & 172.63 ns / 177.53 ns\\
ztrace overhead/call stdev & 2.28 ns\\
ztrace vs uprobe & 11.08x lower overhead\\
install latency avg & 370,539 ns，约 0.371 ms，1000 轮平均\\
uninstall latency avg & 83,892 ns，约 0.084 ms，1000 轮平均\\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{tikzpicture}[
  ...
  \node[font=\small] at (2.3,5.0) {函数调用耗时};
  \draw[axis] (0,0) -- (4.6,0);
  \draw[axis] (0,0) -- (0,4.5);
  ...
  \node[font=\small] at (9.0,5.0) {probe 生命周期延迟};
...
\end{tikzpicture}
\caption{benchmark 柱状图：函数调用开销与 probe 生命周期延迟}
\label{fig:perf-bars}
\end{figure}

本轮结果中，zeroTrace 额外开销约 174.45 ns/call，低于赛题 1000 ns 指标，也低于项目内部设定的 200 ns 优化目标。kernel uprobe 的额外开销均值约 1933.20 ns/call，zeroTrace 在本轮实验中为 11.08x lower overhead。install/uninstall 延迟均低于 1 ms，距离赛题 10 ms 指标还有较大余量。
...
现有代码已经实现函数追踪、返回值捕获、动态开关、多线程控制等赛题要求，提交材料还可以继续补运行记录。下面几项实验优先级较高，分别对应不同平台、不同工具链和长时间运行场景。

\begin{longtable}{@{}P{4.3cm}P{2.4cm}P{8.45cm}@{}}
\caption{后续可补充的实验记录}\label{tab:missing-experiments}\\
\toprule
\textbf{实验} & \textbf{优先级} & \textbf{原因与命令}\\
\midrule
\endfirsthead
\toprule
\textbf{实验} & \textbf{优先级} & \textbf{原因与命令}\\
\midrule
\endhead
aarch64 完整回归 & 高 & A1 已有代码和配置检查，提交材料还应给出 ARM 机器上的真实运行记录：\code{make ARCH=aarch64 test}、\code{make ARCH=aarch64 benchmark}。\\
不同内核/权限环境下的 uprobe 复测 & 中 & 本轮已经在 x86\_64 机器上通过 \code{sudo make benchmark} 跑出 kernel uprobe 对照。后续可在其他 kernel、bpftrace 版本或 tracingfs 配置下复测稳定性。\\
真实 perf/ftrace 合流样例 & 中 & A3 已有格式和脚本自测。后续可采集一段真实 \code{perf script} 或 ftrace \code{sched\_switch}，再用 \code{scripts/merge\_trace\_events.py} 合流。\\
长时间多线程 soak & 中 & 已记录 10 轮压力。正式提交可增加 100 轮或 10 分钟运行日志，观察长时间运行下是否出现偶发死锁和资源泄漏。\\
内存泄漏工具验证 & 中 & 清理测试已检查远程 \code{maps}，但可以补充 \code{valgrind} 或 sanitizers 对 tracer 进程本地资源的验证。\\
不同函数类型开销拆分 & 低 & benchmark 以整数函数为主。可补充 libc 函数、带返回值函数、启用 call action、启用 filter 的分项开销。\\
\bottomrule
\end{longtable}

后续优先补 aarch64 运行记录和真实 perf/ftrace 合流样例。这两项分别回答多架构实测和跨工具时间线分析两个问题。
...
赛题提交材料包括功能实现、设计文档、阶段性汇报、演示视频和最终开源仓库。zeroTrace 仓库包含 \code{LICENSE}，项目自写源代码按 GPLv3 发布；本技术报告、README、架构说明、实验评测说明、答辩幻灯片和演示视频按 CC-BY-SA 4.0 发布。官方技术方案和章程 PDF 作为需求来源存放在 \code{docs/} 中，其授权与解释权归原发布方所有。

章程中的提交节点也会影响材料组织：初赛作品提交截止日为 2026 年 6 月 30 日，阶段性决赛作品提交截止日为 2026 年 8 月 12 日，决赛作品提交截止日暂定为 2026 年 8 月 18 日，现场答辩日暂定为 2026 年 8 月 19 日。源代码、设计方案文档、汇报幻灯片和演示视频需要在各阶段保持一致，决赛结束后相关资料继续公开访问。

提交材料至少包括以下内容：

\begin{enumerate}[leftmargin=2.4em]
  \item 源代码、构建脚本、测试脚本和必要配置文件。
  \item 本技术报告、README、架构说明、实验评测说明和辅助工具使用记录。
  \item 功能、性能、创新性和同类项目对比分析，对应本文的需求覆盖、性能实验、创新性分析和同类方案对比章节。
  \item 进展汇报或答辩幻灯片，以及配套的手动 demo 演示视频。
  \item 能展示持续迭代过程的 git commit 历史，提交信息应说明功能新增、bug 修复、测试补强或性能优化。
  \item 若引用外部资料或往届项目，需要在文档中说明来源、用途、授权信息和本项目新增内容。
\end{enumerate}

\section{讨论}

zeroTrace 已经覆盖赛题的主要要求。条件过滤在 tracer 侧执行，过滤结果只影响日志输出，目标函数仍按原语义执行。A3 提供 perf/ftrace 风格事件格式和合流脚本；实际跨工具分析时，需要确保外部 perf/ftrace 日志使用可对齐的时钟。aarch64 后端已经补上主要代码，但不同 ARM 设备的性能差异很大，报告应单独给出 ARM 机器上的实测结果。

性能优化里有三处改动最直接。stub 记录需要展示的 XMM/FP 快照，用紧凑快照取代全量 \code{fxsave}；payload 直接写 ring buffer slot，省掉从栈上 event 到共享内存的一次复制；trace runner 采用 header-first 与范围 snapshot，序号未变化时停在 header。这些改动保持原有功能语义，同时减少高频调用里的额外工作。
...
本节只说明 AI 辅助工具的使用范围。AI 给出的建议需要经过团队重新整理、审阅和验证后才会进入仓库；项目实现和实验结论以自动化测试、benchmark 输出和提交记录为准。

\section{结论}

zeroTrace 已经打通从安装 probe 到输出日志的完整流程：tracer 负责安装和管理，目标进程内部完成命中后的采集逻辑，事件通过共享 trace buffer 异步传出。项目实现了基础功能 F1--F7，并补充了条件探针、热更新、探针内调用、perf/ftrace 风格日志和多架构后端。x86\_64 benchmark 显示，单次 probe 额外开销保持在 200 ns 以内，安装/卸载延迟远低于 10 ms；与本机 kernel uprobe 对比时，zeroTrace 额外开销低 11.08 倍。提交前还应补上 aarch64 完整运行数据和真实 perf/ftrace 合流样例。
...
\bibitem{winning}
往届获奖项目 README 结构参考：\href{https://gitlab.eduxiji.net/educg-group-35717-2721707/T202518123995935-3027}{GitLab 项目文档页面}
\end{thebibliography}
