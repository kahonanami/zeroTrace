...
需要注意的是，这里的 filter 更新目前只影响 tracer 侧日志消费。payload 侧的 probe 内 call action 会通过共享 trace buffer 中的 `call_actions` 表独立热更新。更完整的 A5 还可以继续覆盖返回值采集策略、日志格式等其他 probe 行为参数。
...
1. 停住目标进程
2. 重新为 probe 安装 trampoline 和入口 patch
3. 继续目标进程
