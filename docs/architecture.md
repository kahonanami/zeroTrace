...
模板最小长度是 14 字节。安装 patch 前会用 Capstone 解析函数开头指令，实际覆盖长度按完整指令累计到不少于模板长度，确保不会截断原始指令。

`aarch64` patch 模板最小长度是 16 字节：
...
trampoline 对于不同的 `original bytes` 具有不同策略。目标是：既不能截断原函数前导指令，也不能把依赖当前位置的指令原样搬走后改变语义。
...
2. 先读取 trace buffer header，只拿 `magic` 和 `write_seq`；如果没有新增事件，不读取事件区
3. 计算本轮需要消费的序列范围 `[last_seq + 1, write_seq]`；如果发生环形覆盖，先跳到当前仍可见的最早序列
4. 只对这段新增范围读取 `committed_seq` before、event 本体和 `committed_seq` after，跨 ring 末尾时拆成两段连续读
5. 只有 `before == event.committed_seq == after == seq` 时才消费该事件
6. 如果 slot 仍未提交，停止本轮 poll 且不推进 `last_seq`，下一轮继续读取同一个 seq，避免读到半写入事件
7. 如果 reader 落后超过 `ZT_TRACE_EVENT_CAPACITY`，日志中写入 `ztrace:lost`，显式记录被覆盖的事件范围，而不是静默跳过
8. 把格式化后的输出追加写入 `ztrace.<pid>.log`，并在 CLI 中重绘显示
9. 如果目标因为信号进入 ptrace stop，则转发该信号并继续目标进程
