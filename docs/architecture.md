...
- 在等待输入时以 `CLOCK_MONOTONIC` 做 50ms 节流轮询共享 trace buffer，并把新增日志重绘到终端
...
- `update <symbol|id> call <symbol|clear> [arg0|arg1|...|arg5|0x...]`
- `stop`
- `continue`
- `info target`
- `info probes`
...
1. 优先使用 `process_vm_readv` 从目标进程读取远程内存；如果失败，则回退到 `ptrace(PTRACE_PEEKDATA)`
2. 先读取 trace buffer header，只拿 `magic` 和 `write_seq`；如果没有新增事件，不读取整块 buffer
3. 从 `last_seq + 1` 开始按 slot 读取事件，并在读取完整 event 后再次读取该 slot 的 `committed_seq`
4. 只有 `event.committed_seq == seq` 且二次读取的 `committed_seq == seq` 时才消费该事件
5. 如果 slot 仍未提交，停止本轮 poll 且不推进 `last_seq`，下一轮继续读取同一个 seq，避免读到半写入事件
6. 如果 reader 落后超过 `ZT_TRACE_EVENT_CAPACITY`，日志中写入 `ztrace:lost`，显式记录被覆盖的事件范围，而不是静默跳过
7. 把格式化后的输出追加写入 `ztrace.<pid>.log`，并在 CLI 中重绘显示
8. 如果目标因为信号进入 ptrace stop，则转发该信号并继续目标进程
9. 如果远程读在目标退出窗口失败，则通过 `kill(pid, 0)`、`/proc/<pid>/stat` 中的 zombie/dead 状态、共享 trace buffer 是否仍出现在 `/proc/<pid>/maps` 中，以及一个短暂确认窗口判断是否属于正常退出；确认退出后关闭 active trace，而不是把它当作 trace 错误
10. 如果刚刚已经观测到目标退出，后续重复调用 `zt_trace_poll()` 返回完成态，避免调用方在边界上多 poll 一次时把正常退出误报为失败
...
`update <symbol|id> call <callee> [arg0|arg1|...|arg5|0x...]` 和 `update <symbol|id> call clear` 属于 payload 侧行为热更新。tracer 会把新的 call action 写入远程 trace buffer 的 `call_actions` 表，payload 后续 entry 命中时直接读取该表决定是否主动调用目标函数。这个更新同样不会重新 patch 函数入口，也不会重新分配 trampoline。

为了避免目标进程正在读取 action 表时 tracer 覆盖字段，call action 更新使用 `enabled` 作为非零 commit token：

1. 先把远程 slot 的 `enabled` 写成 `0`
2. 再写入新的 `probe_id` / `callee_addr` / 参数来源等字段，写入阶段保持 `enabled == 0`
3. 如果是启用 action，最后单独把 `enabled` 写成新的非零 epoch token

payload 读取 action 时会先 acquire 读取一次 token，拷贝本地 snapshot，再 acquire 读取一次 token；只有前后 token 相同且非零时才接受该 snapshot。这样即使 tracer 在运行中清除或切换 call action，payload 也只会看到“旧 action 的完整快照”“新 action 的完整快照”或“disabled”，不会把旧 probe id、新 callee 地址和新参数来源拼成半条状态。
...
`update <symbol|id> call <callee> [args...]` 会给已安装 probe 配置一个 payload 侧动作。tracer 会解析 `<callee>` 在目标进程里的真实地址，并把如下配置写入共享 trace buffer 中的 `call_actions` 表：

- `enabled`
- `probe_id`
- `callee_addr`
- `arg_count`
- `args[]`：每个参数是常量，或引用当前 entry event 中的 `arg0..arg5`

payload 每次处理 entry 事件时，会根据当前 `probe_id` 查询这张 action 表。如果 action 启用，就在目标进程上下文中解析参数并调用对应函数。当前实现支持最多 6 个整型 / 指针参数，对应 x86_64 和 aarch64 ABI 的前 6 个通用参数寄存器：

```c
retval = callee(resolved_arg0, resolved_arg1, ...);
```
...
- `call` 事件保存 probe 内 call action 的目标地址、实际调用参数和返回值
...
tracer 消费事件时不会再复制整块 trace buffer 后盲目推进 `last_seq`。它采用 header-first + per-slot 读取策略：先读取 `write_seq`，再逐个读取需要消费的 slot，并用读取后的 `committed_seq` 二次确认该 slot 没有处在写入中或被环形覆盖。如果遇到未提交 slot，`last_seq` 保持不变；如果落后超过容量，会输出 `ztrace:lost` 记录丢失范围。这样高频 probe 下即使发生 buffer wrap，也能把丢失显式暴露出来，而不是把缺口伪装成正常日志。
